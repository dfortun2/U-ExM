function out = mijread_stack(filename)
% mijread, version 1.0
%
% im = mijread;
% im = mijread(filename);
...
if (nargin == 0)
    ...
    if (name.startsWith('http:') == 1)
        ...
        image=opener.openImage(name);
    end    
end

img=image.getImageStack;
out=zeros(img.getHeight,img.getWidth,img.getSize);
for i=1:img.getSize()
    a=img.getProcessor(i);
    out(:,:,i)=a.getFloatArray()';
end
