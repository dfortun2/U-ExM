function outVol = rotVolClean(inVol,rot,tilt,psi)
...
xRot = xyzRot(:,1)+cx; yRot = xyzRot(:,2)+cy; zRot = xyzRot(:,3)+cz;

xyzRot = bsxfun(@plus,xyzRot,[cx cy cz]);
xRot = xyzRot(:,1); yRot = xyzRot(:,2); zRot = xyzRot(:,3);

%% Rotation - volume
outVol = interp3(inVolPad, yRot, xRot, zRot, 'cubic');
